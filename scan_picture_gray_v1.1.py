...
path_d = {'MainDatabase': "H:\\Databases\\processed_database\\grayscale",
...
pic_scaled = pic.resize((int(pic.size[0] * scale), int(pic.size[1] * scale)), resample=5).convert('L')
...
for j,y in enumerate(bbox_y):
    input_row = np.zeros((len(bbox_x),cfg_model_pic_size,cfg_model_pic_size, 1))
    ...
    for i,x in enumerate(bbox_x):
        ...
        input_row[i] = np.array(pic_part.getdata(), dtype='uint8').reshape(cfg_model_pic_size, cfg_model_pic_size, 1)
